@echo off
REM === VideoRecoder 自动部署脚本 ===
REM 每次构建后运行此脚本，自动复制所有依赖 DLL

set "SRC_DIR=%~dp0"
set "BUILD_DIR=%SRC_DIR%build-VideoRecoder-Desktop_Qt_5_12_11_MinGW_32_bit-Debug\debug"
set "EXE=%BUILD_DIR%\VideoRecoder.exe"
set "WINDEPLOYQT=C:\Qt\Qt5.12.11\5.12.11\mingw73_32\bin\windeployqt.exe"
set "FFMPEG_BIN=%SRC_DIR%VideoRecoder\ffmpeg-4.2.2\ffmpeg-4.2.2\bin"
set "OPENCV_BIN=%SRC_DIR%VideoRecoder\opencv\opencv\opencv-release\opencv-release\bin"

if not exist "%EXE%" (
    echo [ERROR] VideoRecoder.exe not found at %EXE%
...
echo [3/3] Copying OpenCV DLLs...
if exist "%OPENCV_BIN%\*.dll" (
    copy /Y "%OPENCV_BIN%\*.dll" "%BUILD_DIR%\" >nul
) else (
    echo [WARN] OpenCV DLLs not found at %OPENCV_BIN%
)
...
echo VideoRecoder is ready to run.
